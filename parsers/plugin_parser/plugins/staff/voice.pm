...
if ($message =~ /^${sl}${cm}voice $validNick$/i) {
  authCheck($target,$hostname) ? actOut('MESSAGE','chanserv',"voice $target $1") : authError($sender,$target,$target);
}
...
if ($message =~ /^${sl}${cm}devoice $validNick$/i) {
  authCheck($target,$hostname) ? actOut('MESSAGE','chanserv',"devoice $target $1") : authError($sender,$target,$target);
}
