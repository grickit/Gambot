if ($message =~ /^${sl}${cm}ban $validNick ?([0-9]+)?([hmsdw])? ?(.+)?$/i) {
  ...
  if(authCheck($target,$hostname)) {
    ...
    if($banmask ne '') {
      ...
      actOut('LITERAL',undef,"event_schedule>op:$target>delay_schedule>$bantime>send_server_message>chanserv :op $target $botName");
      actOut('LITERAL',undef,"event_schedule>op:$target>delay_schedule>$bantime>event_schedule>op:$target>send_server_message>PRIVMSG $target :Unbanning $1 ($banmask) after $bantime seconds.");
      actOut('LITERAL',undef,"event_schedule>op:$target>delay_schedule>$bantime>event_schedule>op:$target>send_server_message>MODE $target -b *!*\@$banmask");
      actOut('LITERAL',undef,"event_schedule>op:$target>delay_schedule>$bantime>event_schedule>op:$target>send_server_message>PRIVMSG chanserv :deop $target $botName");

      actOut('LITERAL',undef,"event_schedule>op:$target>send_server_message>MODE $target +b *!*\@$banmask");
