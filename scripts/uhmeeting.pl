...
while(defined sleep(1)) {
  ...
  if (($wday < 6) && ($wday > 0) && ($hour == 17) && ($min == 0) && ($sec == 0)) {
    print "send_server_message>PRIVMSG #unknown-horizons :Don't forget about the meeting \x02Sunday\x02 at \x0217:00 UTC\x02! It is currently $hour:$min UTC on $days[$wday].\n";
  }

  elsif (($wday == 6) && (($hour == 12) || ($hour == 14) || ($hour == 16) || ($hour == 18) || ($hour == 20) || ($hour == 22)) && ($min == 0) && ($sec == 0)) {
    print "send_server_message>PRIVMSG #unknown-horizons :Don't forget about the meeting \x02tomorrrow\x02 at \x0217:00 UTC\x02! It is currently $hour:$min UTC.\n";
  }
