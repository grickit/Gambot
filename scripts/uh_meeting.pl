...
if($wday == 0) { ## Sunday
  if($hour == 16) { ## Less than 1 hour remaining
    if($min == 0) {
      $core->server_send("PRIVMSG #unknown-horizons :Don't forget about the meeting \x02today\x02 in \x02$mins_off minutes\x02! [ http://git.io/9vk4bg ]");
    ...
    $core->server_send("PRIVMSG #unknown-horizons :Don't forget about the meeting \x02today\x02 in \x02$time_left hours\x02! [ http://git.io/9vk4bg ]");
  ...
  if($hour == 12 || $hour == 16 || $hour == 20) {
    $core->server_send("PRIVMSG #unknown-horizons :Don't forget about the meeting \x02tomorrrow\x02 at \x0217:00 UTC\x02! For reference, it is currently $hour:$min UTC. [ http://git.io/9vk4bg ]");
  ...
  if($hour == 17) {
    $core->server_send("PRIVMSG #unknown-horizons :Don't forget about the meeting \x02Sunday\x02 at \x0217:00 UTC\x02! For reference, it is currently $hour:$min UTC on $days[$wday]. [ http://git.io/9vk4bg ]");
